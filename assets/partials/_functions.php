<?php
    function db_connect()
    {
        ...
        $conn = mysqli_connect($servername, $adminname, $password, $database);
        return $conn;
    }

    function exist_admin($conn, $adminname)
    {
    ...
    function is_bus_assigned($conn, $busno, $depdate) {
        $sql = "SELECT * FROM `routes` WHERE `bus_no` = '$busno' AND `route_dep_date` = '$depdate'";
        $result = mysqli_query($conn, $sql);
